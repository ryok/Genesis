...
def create_height_field_terrain_with_recording(terrain_type: str, difficulty: int, 
                                               save_video: bool = True, video_dir: str = "videos"):
    """高さフィールド地形を作成し、動画記録を設定"""
...
def evaluate_terrain_with_video_recording(terrain_type: str, difficulty: int, 
    ...
    # シーン作成（動画記録付き）
    print("地形を生成中...")
    scene, video_path = create_height_field_terrain_with_recording(
...
def main():
    parser = argparse.ArgumentParser(
        description="地形汎化性能評価ツール（動画記録版）",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用例:
  # 動画記録付きでシンプル歩行ポリシーを評価
  python terrain_evaluation_video_recorder.py --terrain steps --difficulty 2 --save_video
  
  # 学習済みポリシーで動画記録
  python terrain_evaluation_video_recorder.py --terrain steps --difficulty 2 \\
      --policy logs/go2-walking/model_100.pt --save_video --video_dir experiment_videos
  
  # 動画記録なしで評価のみ
  python terrain_evaluation_video_recorder.py --terrain slopes --difficulty 1
        """
    )
