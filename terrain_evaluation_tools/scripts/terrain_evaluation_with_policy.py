...
def load_policy(policy_path: str, config_path: str):
    """学習済みポリシーを読み込み"""
    ...
    try:
        ...
        # ポリシーを読み込み
        runner = OnPolicyRunner(None, train_cfg, "", device=gs.device)
        runner.load(policy_path)
        policy = runner.get_inference_policy(device=gs.device)
        
        print(f"✓ ポリシーを読み込みました: {policy_path}")
        
        return policy, env_cfg, obs_cfg
        
    except ImportError as e:
        raise ImportError(
            "RSL-RLライブラリが見つかりません。以下でインストールしてください:\n"
...
def evaluate_terrain_with_policy(terrain_type: str, difficulty: int, policy_path: str = None, 
                                config_path: str = None, max_steps: int = 2000):
    """学習済みポリシーで地形評価を実行"""
    ...
    # ポリシーを読み込み
    if policy_path and config_path:
        try:
            policy, env_cfg, obs_cfg = load_policy(policy_path, config_path)
    ...
    # ダミー観測（学習済みポリシー用）
    dummy_obs = torch.zeros(45, device=gs.device)  # Go2の標準観測次元
    ...
    try:
        for step in range(max_steps):
            ...
            if use_trained_policy:
                # 学習済みポリシーを使用
                # 実際の観測を構築する必要があるが、ここでは簡易版
                actions = policy(dummy_obs.unsqueeze(0))[0]
            else:
                # シンプルポリシーを使用
...
def main():
    parser = argparse.ArgumentParser(
        description="地形汎化性能評価ツール（ポリシー対応版）",
...
使用例:
  ...
  python terrain_evaluation_with_policy.py --terrain slopes --all_levels \\
    ...
    if args.policy and not args.config:
        parser.error("--policy を指定する場合は --config も必要です")
    
    if args.config and not args.policy:
        parser.error("--config を指定する場合は --policy も必要です")
    ...
    if args.all_levels:
        ...
        for level in [1, 2, 3]:
            results[level] = evaluate_terrain_with_policy(
                args.terrain, level, args.policy, args.config, args.max_steps
            )
        ...
        result = evaluate_terrain_with_policy(
            args.terrain, args.difficulty, args.policy, args.config, args.max_steps
        )
